...
definePageMeta({
  layout: 'default-terracotta',
  middleware: ["auth"]
})

const { update, find, findOne } = useStrapi();
const { data: guest } = await find("guest-syukurans", {
...
const getDetail = async () => {
  detail.value = await find("syukuran-detail", {
...
const open = async () => {
  ...
  const res = await update("guest-syukurans-data", route.params.guest, {
    is_open: true,
  });
};
...
const colorTheme = ref('terracotta')
...
<template>
  <div class="w-full overflow-x-hidden relative overflow-y-hidden">
    <div ref="cover" class="fixed z-50 cover-invitation overflow-hidden inset-0 transition-all duration-1000">
      <CoverSyukuran :initCategory="categoryIndex" :guest="invitation" :category="categories" @openInvitation="open"
        @play="playNewMusic" />
    </div>
    <div>
      ...
      <div class="fixed bottom-12 z-40 right-3">
        ...
        <div class="flex flex-col gap-3" v-show="!isFirstPlay">
          <div @click="openPlaylist" :class="`
                          cursor-pointer
                          bg-white
                          ${colorTheme === 'terracotta' ? 'text-primary-terracotta border-primary-terracotta' : 'text-primary-light border-primary'}
                          border
                          p-3
                          rounded-full
                          h-12
          ...
          <div @click="toggleShuffle" :class="`
                        ...
                        `">
            <Shuffle />
            <div v-if="isShuffle" :class="`w-1 h-1 ${colorTheme === 'terracotta' ? 'bg-primary-terracotta' : 'bg-primary-light'} mx-auto rounded-full`"></div>
          </div>
          <div @click="toggleAudio" :class="`
                          h-12
                          w-12
                          ${colorTheme === 'terracotta' ? 'bg-primary-terracotta' : 'bg-primary-light'}
      ...
      <SidePlaylist :color="colorTheme" @select="selectSong" :currentCategory="currentCategory" :selected-category="selectedCategory"
      ...
      <transition name="fade">
        <div v-show="isPlay" :class="`
                        ...
                        px-4
                        py-1
                        ${colorTheme === 'terracotta' ? 'bg-primary-terracotta/80' : 'bg-primary-light/80'}
    ...
    <div>
      <div
        :class="`fixed transition-all duration-500 border-2 ${toastType === 'success' ? (colorTheme === 'terracotta' ? 'border-primary-terracotta' : 'border-primary-light') : (toastType === 'danger' ? 'border-red-700' : 'border-gray-700')} ${toastShow ? 'scale-100 opacity-100' : 'scale-0 opacity-0'} z-100 top-8 inset-x-0 w-5/6 shadow-lg md:w-full max-w-md lg:max-w-xl bg-white p-2 gap-1 rounded-xl flex mx-auto items-center`">
        <div>
          <svg v-if="toastType === 'success'" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
            stroke-width="1.5" stroke="currentColor" :class="`w-8 h-8 ${colorTheme === 'terracotta' ? 'text-primary-terracotta' : 'text-primary-light'}`">
            <path stroke-linecap="round" stroke-linejoin="round"
